...
​                --添加学生表数据 --添加课程表数据 --添加成绩表数据 --删除成绩表数据

**3，添加字段、更改字段**

​                --往学生表里追加生日字段 --更改成绩表中的成绩字段为number(18,2)类型
...
​                --查出成绩表与成绩备份表不重复记录 --查出成绩表与成绩备份表所有记录
...
# 数据同步，同步成绩中的数据到备份表，成绩表中不存在的则在备份表中删除，成绩表中存在且备份表中也存在则更新，成绩表中存在备份表中不存在则进行插入
